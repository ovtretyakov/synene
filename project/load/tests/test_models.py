...
class CommonHandlerModelTest(TestCase):
    ...
    def test_common_handler_save_match_stat(self):
        ...
        stat = MatchStats.get_object(match=match,stat_type=Match.GOALS,competitor=Match.COMPETITOR_HOME,period=0)
        self.assertEquals(stat.value, '2')
        stat = MatchStats.get_object(match=match,stat_type=Match.GOALS,competitor=Match.COMPETITOR_HOME,period=1)
        self.assertEquals(stat.value, '2')
        stat = MatchStats.get_object(match=match,stat_type=Match.GOALS,competitor=Match.COMPETITOR_HOME,period=2)
        self.assertEquals(stat.value, '0')
        stat = MatchStats.get_object(match=match,stat_type=Match.GOALS,competitor=Match.COMPETITOR_AWAY,period=0)
        self.assertEquals(stat.value, '3')
        stat = MatchStats.get_object(match=match,stat_type=Match.GOALS,competitor=Match.COMPETITOR_AWAY,period=1)
        self.assertEquals(stat.value, '1')
        stat = MatchStats.get_object(match=match,stat_type=Match.GOALS,competitor=Match.COMPETITOR_AWAY,period=2)
        self.assertEquals(stat.value, '2')
        match.refresh_from_db()
        self.assertEquals(match.score, '2:3 (2:1,0:2)')
        stat = MatchStats.get_object(match=match,stat_type=Match.XG,competitor=Match.COMPETITOR_HOME,period=0)
        self.assertIsNone(stat)
        stat = MatchStats.get_object(match=match,stat_type=Match.XG,competitor=Match.COMPETITOR_AWAY,period=0)
        self.assertIsNone(stat)
        stat = MatchStats.get_object(match=match,stat_type=Match.XG,competitor=Match.COMPETITOR_HOME,period=1)
        self.assertIsNone(stat)
        stat = MatchStats.get_object(match=match,stat_type=Match.XG,competitor=Match.COMPETITOR_AWAY,period=1)
        self.assertIsNone(stat)
        stat = MatchStats.get_object(match=match,stat_type=Match.YCARD,competitor=Match.COMPETITOR_HOME,period=0)
        self.assertIsNone(stat)
        stat = MatchStats.get_object(match=match,stat_type=Match.YCARD,competitor=Match.COMPETITOR_AWAY,period=0)
        self.assertIsNone(stat)
        stat = MatchStats.get_object(match=match,stat_type=Match.YCARD,competitor=Match.COMPETITOR_HOME,period=1)
        self.assertIsNone(stat)
        stat = MatchStats.get_object(match=match,stat_type=Match.YCARD,competitor=Match.COMPETITOR_AWAY,period=1)
        self.assertIsNone(stat)
        stat = MatchStats.get_object(match=match,stat_type=Match.RCARD,competitor=Match.COMPETITOR_HOME,period=0)
        self.assertIsNone(stat)
        stat = MatchStats.get_object(match=match,stat_type=Match.RCARD,competitor=Match.COMPETITOR_AWAY,period=0)
        self.assertIsNone(stat)
        stat = MatchStats.get_object(match=match,stat_type=Match.RCARD,competitor=Match.COMPETITOR_HOME,period=2)
        self.assertIsNone(stat)
        stat = MatchStats.get_object(match=match,stat_type=Match.RCARD,competitor=Match.COMPETITOR_AWAY,period=2)
        self.assertIsNone(stat)
        stat = MatchStats.get_object(match=match,stat_type=Match.PENALTY,competitor=Match.COMPETITOR_HOME,period=0)
        self.assertIsNone(stat)
        stat = MatchStats.get_object(match=match,stat_type=Match.PENALTY,competitor=Match.COMPETITOR_AWAY,period=0)
        self.assertIsNone(stat)
        stat = MatchStats.get_object(match=match,stat_type=Match.PENALTY,competitor=Match.COMPETITOR_HOME,period=2)
        self.assertIsNone(stat)
        stat = MatchStats.get_object(match=match,stat_type=Match.PENALTY,competitor=Match.COMPETITOR_AWAY,period=2)
        self.assertIsNone(stat)
        stat = MatchStats.get_object(match=match,stat_type=Match.GOALS_MINUTE,competitor=Match.COMPETITOR_HOME,period=15)
        self.assertIsNone(stat)
        stat = MatchStats.get_object(match=match,stat_type=Match.GOALS_MINUTE,competitor=Match.COMPETITOR_AWAY,period=30)
        self.assertIsNone(stat)
        stat = MatchStats.get_object(match=match,stat_type=Match.GOAL_TIME,competitor=Match.COMPETITOR_HOME,period=0)
        self.assertIsNone(stat)
        stat = MatchStats.get_object(match=match,stat_type=Match.GOAL_TIME,competitor=Match.COMPETITOR_AWAY,period=0)
        self.assertIsNone(stat)
        stat = MatchStats.get_object(match=match,stat_type=Match.SHOTS,competitor=Match.COMPETITOR_HOME,period=0)
        self.assertIsNone(stat)
        stat = MatchStats.get_object(match=match,stat_type=Match.SHOTS,competitor=Match.COMPETITOR_AWAY,period=0)
        self.assertIsNone(stat)
        stat = MatchStats.get_object(match=match,stat_type=Match.SHOTS_ON_TARGET,competitor=Match.COMPETITOR_HOME,period=0)
        self.assertIsNone(stat)
        stat = MatchStats.get_object(match=match,stat_type=Match.SHOTS_ON_TARGET,competitor=Match.COMPETITOR_AWAY,period=0)
        self.assertIsNone(stat)
        stat = MatchStats.get_object(match=match,stat_type=Match.DEEP,competitor=Match.COMPETITOR_HOME,period=0)
        self.assertIsNone(stat)
        stat = MatchStats.get_object(match=match,stat_type=Match.DEEP,competitor=Match.COMPETITOR_AWAY,period=0)
        self.assertIsNone(stat)
        stat = MatchStats.get_object(match=match,stat_type=Match.PPDA,competitor=Match.COMPETITOR_HOME,period=0)
        self.assertIsNone(stat)
        stat = MatchStats.get_object(match=match,stat_type=Match.PPDA,competitor=Match.COMPETITOR_AWAY,period=0)
        self.assertIsNone(stat)
        stat = MatchStats.get_object(match=match,stat_type=Match.CORNERS,competitor=Match.COMPETITOR_HOME,period=0)
        self.assertIsNone(stat)
        stat = MatchStats.get_object(match=match,stat_type=Match.CORNERS,competitor=Match.COMPETITOR_AWAY,period=0)
        self.assertIsNone(stat)
        stat = MatchStats.get_object(match=match,stat_type=Match.FOULS,competitor=Match.COMPETITOR_HOME,period=0)
        self.assertIsNone(stat)
        stat = MatchStats.get_object(match=match,stat_type=Match.FOULS,competitor=Match.COMPETITOR_AWAY,period=0)
        self.assertIsNone(stat)
        stat = MatchStats.get_object(match=match,stat_type=Match.OFFSIDES,competitor=Match.COMPETITOR_HOME,period=0)
        self.assertIsNone(stat)
        stat = MatchStats.get_object(match=match,stat_type=Match.OFFSIDES,competitor=Match.COMPETITOR_AWAY,period=0)
        self.assertIsNone(stat)
        stat = MatchStats.get_object(match=match,stat_type=Match.POSSESSION,competitor=Match.COMPETITOR_HOME,period=0)
        ...
        stat = MatchStats.get_object(match=match,stat_type=Match.GOALS_MINUTE,competitor=Match.COMPETITOR_HOME,period=15)
        self.assertEquals(stat.value, '1')
        stat = MatchStats.get_object(match=match,stat_type=Match.GOALS_MINUTE,competitor=Match.COMPETITOR_AWAY,period=75)
        self.assertEquals(stat.value, '3')
        stat = MatchStats.get_object(match=match,stat_type=Match.XG_MINUTE,competitor=Match.COMPETITOR_HOME,period=30)
        self.assertEquals(stat.value, '0.300')
        stat = MatchStats.get_object(match=match,stat_type=Match.XG_MINUTE,competitor=Match.COMPETITOR_AWAY,period=90)
